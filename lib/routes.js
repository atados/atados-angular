...
var serveSitemap = function(req, res) {
  console.log(config.api_host_url + '/sitemap.xml')
  req.pipe(request(config.api_host_url + '/sitemap.xml')).pipe(res);
}
/**
 * Application routes
 */
module.exports = function(app) {
  ...
  app.get('/vaga/:slug', function(req, res) {
    request(config.api_url + '/project/' + req.params.slug + '/?format=json', (err, response, body) => {
      ...
      return renderApp({
        title: project.name + ' | ' + config.title,
        initialState: {
          project: project
        },
        metas: [
          {
            name: 'title',
            content: project.name + ' | ' + config.title,
            repeat_with_prefix: ['og:']
          },
          {
            name: 'description',
