...
// factory
function Auth ($http, $cookieStore, Cleanup) {
  'ngInject';
  ...
  return {
    facebookAuth: function (facebookAuthData, success, error) {
      $http.post(api + 'facebook/', facebookAuthData).success(function (response) {
        ...
        $cookieStore.put(accessTokenCookie, response.access_token);
    ...
    getCurrentUser: function (token) {
      if (!token) {
        token = $cookieStore.get(accessTokenCookie);
      }
        ...
        .success(function (response) {
          ...
          $http({
            ...
            data: param(user)
              ...
              if (user.remember) {
                $cookieStore.put(accessTokenCookie, response.access_token, { expires: 30, path: '/' });
              }
